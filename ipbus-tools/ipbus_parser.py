class Packet:
    ...
    @classmethod
    def bytes_to_le_word(cls, bytes):
        if len(bytes) % 4 != 0:
...
class Transaction:
    ...
    def __repr__(self) -> str:
        ...
        for (word_label, word) in zip(self.word_labels, self.words):
            result += f"{word_label}:\t0x{word:08x}\n"
        return result

    @classmethod
    def from_le_bytes(cls, bytes):
        header = TransactionHeader.from_le_bytes(bytes[0:4])
        words = Packet.bytes_to_le_word(bytes[4:])
        return cls(header, words, header.get_payload_fields())
